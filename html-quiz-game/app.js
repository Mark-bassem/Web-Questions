(() => {
  ...
  // Supabase shared leaderboard (for GitHub Pages)
  // IMPORTANT: put your keys from Supabase Project Settings
  // SUPABASE_URL  : Project Settings → API → Project URL
  // SUPABASE_ANON_KEY: Project Settings → API → anon public key
  // انسخهم كما هم (بدون علامات الاقتباس)
  const SUPABASE_URL = "https://qttfzeafpzbdxfaxnmwj.supabase.co/rest/v1/"; // e.g. https://xxxxxx.supabase.co
  const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InF0dGZ6ZWFmcHpiZHhmYXhubXdqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzkyMTQ2NzgsImV4cCI6MjA5NDc5MDY3OH0.NZBeoW4BZyF-HqbgmcVZQRceS44FiyTWYo7IA6N3_ac"; // e.g. eyJhbGciOi...
  ...
  const supabaseEnabled = Boolean((SUPABASE_URL || "").trim() && (SUPABASE_ANON_KEY || "").trim());

  const supabaseFetch = async (path, { method = "GET", headers = {}, body = null } = {}) => {
  ...
  const getLeaderboardRemote = async () => {
    // Query: top 10 sorted by score desc then time asc (oldest first for same score)
    // Using PostgREST: /rest/v1/{table}?select=name,score,time&order=score.desc,time.asc&limit=10
    // Also cast time as bigint already.
    const data = await supabaseFetch(
      `/rest/v1/${encodeURIComponent(SUPABASE_TABLE)}?select=name,score,time&order=score.desc,time.asc&limit=10`,
      { method: "GET" }
    );
  ...
  const saveResultToLeaderboardRemote = async ({ name, score, time }) => {
    // Insert-only: every result becomes a new row.
    // Table columns must be (name, score, time)
    await supabaseFetch(`/rest/v1/${encodeURIComponent(SUPABASE_TABLE)}`, {
      method: "POST",
      headers: {
        Prefer: "return=representation"
      },
      body: [{ name, score, time }]
    });
  };
  ...
  const showLeaderboard = async () => {
    ...
    el.leaderBody.innerHTML = `<tr><td colspan="3" style="color:var(--muted);text-align:center;">جارٍ التحميل...</td></tr>`;

    try {
      let rows = [];
      if (supabaseEnabled) {
        rows = await getLeaderboardRemote();
      }

      // Fallback to localStorage if remote is not configured or failed.
      if (!rows.length) {
        rows = getLeaderboard()
          .slice()
      ...
      // On any error show localStorage.
  ...
  const saveResultToLeaderboard = async () => {
    ...
    // Local optimistic fallback (so user sees it even if remote misconfigured).
    try {
      const rows = getLeaderboard();
      rows.push({ name, score, time: now });
      saveLeaderboard(rows.slice(-200));
    } catch {}

    // Remote insert (shared across devices).
    try {
      if (supabaseEnabled) {
        await saveResultToLeaderboardRemote({ name, score, time: now });
      }
    } catch {}
